...
(function(){
	...
	function build() {
			...
			this.setContext = function(ctx) {
				...
				if (typeof(ctx.font) != 'undefined') {
					ctx.font = svg.Font.CreateFont(
						...
						this.style('fonts-weight').value,
						this.style('fonts-size').hasValue() ? this.style('fonts-size').Length.toPixels() + 'px' : '',
						this.style('fonts-family').value).toString();
				}
		...
		svg.Element.font = function(node) {
			...
			for (var i=0; i<this.children.length; i++) {
				...
				if (child.type == 'fonts-face') {
					this.fontFace = child;
					if (child.style('fonts-family').hasValue()) {
						svg.Definitions[child.style('fonts-family').value] = this;
					}
				}
		...
		svg.Element.TextElementBase = function(node) {
			...
			this.renderChildren = function(ctx) {
				var customFont = this.parent.style('fonts-family').Definition.getDefinition();
				if (customFont != null) {
					var fontSize = this.parent.style('fonts-size').numValueOrDefault(svg.Font.Parse(svg.ctx.font).fontSize);
					...
					for (var j=0; j<cssClasses.length; j++) {
						...
						if (cssClass != '') {
							...
							if (cssClass == '@fonts-face') {
								var fontFamily = props['fonts-family'].value.replace(/"/g,'');
